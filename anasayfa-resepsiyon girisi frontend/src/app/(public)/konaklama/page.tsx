...
export default function KonaklamaPage() {
  ...
  const [odaTipi, setOdaTipi] = useState('Standart');
  const [notlar, setNotlar] = useState('');

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    router.push('/oda-secimi');
  };
      ...
      <div className="flex-1 flex flex-col relative z-10">
        ...
        <div className="flex-1 p-12 flex flex-col items-center relative z-10 overflow-y-auto">
          ...
          <div className="bg-white w-full max-w-[900px] rounded-xl p-8 md:px-10 shadow-[0_8px_24px_rgba(0,0,0,0.04)] relative z-10">
            ...
            <div className="mb-8">
              <h2 className="text-[1.35rem] font-semibold text-[#1a3b25] mb-1.5">Konaklama Detayları</h2>
              <p className="text-[0.9rem] text-[#888]">Giriş/çıkış tarihlerini ve kişi sayısını belirleyin.</p>
            </div>
            ...
            <form onSubmit={handleSubmit}>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 gap-x-8 mb-8">
                ...
                <div className="flex flex-col">
                  <label className="text-[0.9rem] font-semibold text-[#555] mb-2"><span className="text-[#d9534f] mr-1">*</span>Giriş Tarihi</label>
                  <div className="relative">
                    <input type="date" value={girisTarihi} onChange={(e) => setGirisTarihi(e.target.value)} required className="w-full bg-[#2b5b3b] text-white border-none rounded-md py-3 px-[15px] pr-10 text-[0.95rem] outline-none transition-shadow focus:shadow-[0_0_0_2px_#4caf50]" style={{ colorScheme: 'dark' }} />
                    <svg className="absolute right-[15px] top-1/2 -translate-y-1/2 w-[18px] h-[18px] fill-[#a8c1b1] pointer-events-none" viewBox="0 0 24 24"><path d="M19 3h-1V1h-2v2H8V1H6v2H5c-1.11 0-1.99.9-1.99 2L3 19c0 1.1.89 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm0 16H5V8h14v11z"/></svg>
                  </div>
                </div>

                <div className="flex flex-col">
                  <label className="text-[0.9rem] font-semibold text-[#555] mb-2"><span className="text-[#d9534f] mr-1">*</span>Çıkış Tarihi</label>
                  <div className="relative">
                    <input type="date" value={cikisTarihi} onChange={(e) => setCikisTarihi(e.target.value)} required className="w-full bg-[#2b5b3b] text-white border-none rounded-md py-3 px-[15px] pr-10 text-[0.95rem] outline-none transition-shadow focus:shadow-[0_0_0_2px_#4caf50]" style={{ colorScheme: 'dark' }} />
                    <svg className="absolute right-[15px] top-1/2 -translate-y-1/2 w-[18px] h-[18px] fill-[#a8c1b1] pointer-events-none" viewBox="0 0 24 24"><path d="M19 3h-1V1h-2v2H8V1H6v2H5c-1.11 0-1.99.9-1.99 2L3 19c0 1.1.89 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm0 16H5V8h14v11z"/></svg>
                  </div>
                </div>

                <div className="flex flex-col">
                  <label className="text-[0.9rem] font-semibold text-[#555] mb-2"><span className="text-[#d9534f] mr-1">*</span>Yetişkin Sayısı</label>
                  <input type="number" min="1" value={yetiskin} onChange={(e) => setYetiskin(e.target.value)} required className="w-full bg-[#2b5b3b] text-white border-none rounded-md py-3 px-[15px] text-[0.95rem] outline-none transition-shadow focus:shadow-[0_0_0_2px_#4caf50]" />
                </div>

                <div className="flex flex-col">
                  <label className="text-[0.9rem] font-semibold text-[#555] mb-2">Çocuk Sayısı</label>
                  <input type="number" min="0" value={cocuk} onChange={(e) => setCocuk(e.target.value)} className="w-full bg-[#2b5b3b] text-white border-none rounded-md py-3 px-[15px] text-[0.95rem] outline-none transition-shadow focus:shadow-[0_0_0_2px_#4caf50]" />
                </div>

                <div className="flex flex-col">
                  <label className="text-[0.9rem] font-semibold text-[#555] mb-2"><span className="text-[#d9534f] mr-1">*</span>Konaklama Tipi</label>
                  <div className="relative">
                    <select value={odaTipi} onChange={(e) => setOdaTipi(e.target.value)} required className="w-full bg-white text-[#333] border border-[#ddd] rounded-md py-3 px-[15px] pr-10 text-[0.95rem] outline-none appearance-none cursor-pointer transition-colors focus:border-[#2b5b3b]">
                      <option value="Standart">Standart</option>
                      <option value="Suit">Suit</option>
                      <option value="Aile">Aile Odası</option>
                    </select>
                    <svg className="absolute right-[15px] top-1/2 -translate-y-1/2 w-[18px] h-[18px] fill-[#888] pointer-events-none" viewBox="0 0 24 24"><path d="M7.41 8.59L12 13.17l4.59-4.58L18 10l-6 6-6-6 1.41-1.41z"/></svg>
                  </div>
                </div>

                <div className="flex flex-col">
                  <label className="text-[0.9rem] font-semibold text-[#555] mb-2">Özel İstek / Notlar</label>
                  <textarea value={notlar} onChange={(e) => setNotlar(e.target.value)} placeholder="Ekstra yastık, engelli oda gereksinimi vb." className="w-full bg-[#2b5b3b] text-white border-none rounded-md py-3 px-[15px] text-[0.95rem] outline-none min-h-[47px] h-[47px] resize-y transition-shadow focus:shadow-[0_0_0_2px_#4caf50] placeholder:text-[#a8c1b1]"></textarea>
                </div>
              ...
              <div className="flex justify-between items-center relative">
                <Link href="/dashboard" className="bg-white border border-[#ccc] text-[#555] py-2.5 px-6 rounded-md text-[0.9rem] font-medium inline-flex items-center gap-2 transition-colors hover:bg-[#f0f0f0]">
                  ← Geri
                </Link>
